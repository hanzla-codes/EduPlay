...
const LEVEL_DESCRIPTIONS = {
    beginner: {
        ...
        description: "Focuses on the core syntax, basic tags, fundamental CSS properties, and simple JavaScript variables. Get ready to build your base!"
    },
    intermediate: {
        title: "Intermediate Level: Practical Application",
        description: "Tests your understanding of layout (Flexbox/Grid), asynchronous JS (Fetch), and scope/loops. These are essential for modern web development."
    },
    advanced: {
        title: "Advanced Level: Deep Dive",
        description: "Challenges your knowledge of complex JS concepts like Closures, Call Stack, REST methods (POST), and advanced CSS units (rem). Prove your mastery!"
    }
};
...
function logout() {
    ...
    showLandingPage();
    updateInfoMessage(appHeader, 'You have been logged out successfully. Please login again to continue.', true);
}
...
function register() {
    ...
    if (usernameInput.length < 3 || pass.length < 6) {
        updateInfoMessage(registerMsg, 'Username must be at least 3 chars. Password must be at least 6 chars.');
        return;
    }
...
function loadQuestion() {
    ...
    levelText.textContent = `${currentLevel.toUpperCase()} Level (Points per correct answer: ${LEVEL_POINTS[currentLevel]})`;
...
function submitAnswer() {
    ...
    let isCorrect = false;
    const sanitizedAnswer = currentQuestion.answer.replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g,"").toLowerCase().trim();
    const sanitizedUserAnswer = userAnswer.replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g,"").toLowerCase().trim();
...
function handleLevelCompletion() {
    ...
    if (currentLevel === 'beginner') {
        completionMessage = '<b>Beginner Level Complete!</b> Your foundational score is recorded. Would you like to proceed?';
    } else if (currentLevel === 'intermediate') {
        completionMessage = '<b>Intermediate Level Complete!</b> Challenge accepted. Ready for the Advanced questions?';
    ...
    continueBtn.textContent = `Continue to ${currentLevel === 'beginner' ? 'Intermediate' : 'Advanced'}`;
...
function getRemarks(totalScore, maxScore) {
    ...
    if (percentage >= 50) return "Good effort. Keep practicing the advanced concepts.";
...
function showResult() {
    ...
    const maxIntermediate = quizData.intermediate.length * LEVEL_POINTS.intermediate;
    ...
    scoreText.innerHTML = `
        <h3 style="color: var(--primary-color);">Final Scorecard for ${user || 'User'}</h3>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 20px;">
            ...
            <tbody>
                <tr>
                    ...
                    <td style="padding: 10px; border: 1px solid #e0e0e0;">${levelScores.beginner}</td>
                ...
                <tr>
                    ...
                    <td style="padding: 10px; border: 1px solid #e0e0e0;">${levelScores.intermediate}</td>
                ...
                <tr>
                    ...
                    <td style="padding: 10px; border: 1px solid #e0e0e0;">${levelScores.advanced}</td>
